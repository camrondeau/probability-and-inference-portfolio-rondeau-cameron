...
for(i in 1:1000){
  arithmetic_ln[i] = mean(rlnorm(100,-1,1))
  geometric_ln[i] = exp(mean(log(rlnorm(100,-1,1))))
  }
...
for(i in 1:1000){
  arithmetic_u[i] = mean(runif(100,0,12))
  geometric_u[i] = exp(mean(log(runif(100,0,12))))
  }
...
When we looked at the scatter plot of the arithmetic mean and geometric mean, it was clear that every point on the plot fell on one side or the other of line of identity. Therefore, that means that the arithmetic mean or geometric mean is always smaller or larger than the other. Therefore, we can conclude that E[log (X)] is never equal to log (E[X]). From the scatter plots I generated for gamma, log normal, and uniform, it was also clear that the majority of the points fell below the line of identity, meaning the arithmetic mean was almost always bigger than the geometric mean, but never equal. There were also instances where geometric was bigger than arithmetic.
