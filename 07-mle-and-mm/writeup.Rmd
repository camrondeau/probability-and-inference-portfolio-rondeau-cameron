...
y <- quantile(rnorm(1000, mean_gh, sd_gh), probs = (1:1000)/1000)
...
y <- quantile(rnorm(1000, mean_ht, sd_ht), probs = (1:1000)/1000)
plot(x,y, asp=1, main="QQ Plot for Height")
abline(0,1)
```

As you can see above, the points on the Glycohemoglobin QQ plot generally fall on the y=x line, but towards both ends the points start to stray a decent amount, indicating that the normal distribution may not be the best fit, which we also saw in the CDF and PDF above. 

The QQ plot for height is much more uniform, and more points fall closer to the y=x line, indicating the normal distribution is a good fit, which also backs up the CDF and PDF plots above.
...
y <- quantile(rgamma(1000, shape=shape_gh, scale=scale_gh), probs = (1:1000)/1000)
plot(x,y, asp=1, main="QQ Plot for Glycohemoglobin")
abline(0,1)
...
y <- quantile(rgamma(1000, shape=shape_ht, scale=scale_ht), probs = (1:1000)/1000)
plot(x,y, asp=1, main="QQ Plot for Height")
abline(0,1)
```
...
y <- quantile(rweibull(1000, shape=shape_gh, scale=scale_gh), probs = (1:1000)/1000)
plot(x,y, asp=1, main="QQ Plot for Glycohemoglobin")
abline(0,1)
...
y <- quantile(rweibull(1000, shape=shape_ht, scale=scale_ht), probs = (1:1000)/1000)
plot(x,y, asp=1, main="QQ Plot for Height")
abline(0,1)
```

As you can see above, the points on the Glycohemoglobin QQ plot generally fall on the y=x line, but towards both ends the points start to stray a decent amount, indicating that the weibull distribution may not be the best fit, which we also saw in the CDF and PDF above. The points on the height line don't line up well at all.
...
Now, I will use method of moments to show how to estimate parameters of the weibull distribution. I know that for the gamma distribution, mean = shape * scale, and variance = shape * $scale^2$. Therefore, we can calculate that scale = variance / mean, and shape = mean$^2$ / variance. We can do this for both Glycohemoglobin and height separately, which I have done in the code below.
